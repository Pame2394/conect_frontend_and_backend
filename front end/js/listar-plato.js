const tabla = document.getElementById('tbl-platillos');
const cuerpoTabla = document.querySelector('#tbl-platillos tbody');
...
const llenarArregloUsuarios = async() => {
    usuarios = await getDatos('listar-plato');
    llenarTabla();
};
...
const llenarTabla = () => {
    ...
    usuarios.forEach(usuarioTemp => {
        if (usuarioTemp.nombre.toLowerCase().includes(inputFiltro.value.toLowerCase())) {
            ...
            fila.insertCell().textContent = usuarioTemp.ingredientes;
            fila.insertCell().textContent = usuarioTemp.precio;
...
document.getElementById('btn-agregar').addEventListener('click', () => {
    window.location.href = 'registrar-plato.html';
});